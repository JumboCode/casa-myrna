...
const MyCalendar = (props: {}) => {
  // const { events, updateInfo, setUpdateInfo } = props; // Extract events from props
  ...
  const handleSubmit = async (e: any ) => {
      ...
      // let shiftJSON = JSON.stringify(updateInfo)
              ...
              <Grid container spacing={2} columnSpacing={{ xs: 10, sm: 80, md: 5, lg: 5 }} justify-content='flex-start' alignItems='flex-start' columns={12} margin={{ xs: 1, sm: 2, md: 3, lg: 4 }} sx={{width:"100%"}}>
                <Grid container spacing={4} direction='column' alignItems='flex-start' paddingBottom='13%' sx={{width: "100%"}}>
                  ...
                  <Grid container direction="row" sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: -1, width:"100%" }}>
                    ...
                    {/* <Box sx={{ flex: 1, border: 1, borderColor:updateInfo?.style.backgroundColor == "gray" ? "red" : updateInfo?.style.backgroundColor, borderRadius: "5px", width: "100%", color: updateInfo?.style.backgroundColor == "gray" ? "red" : updateInfo?.style.backgroundColor }}> <Typography sx={{display: "flex", alignItems: "center", justifyContent: "center"}}> {updateInfo?.style.backgroundColor == "green" ? "Assigned" : (updateInfo?.style.backgroundColor == "gray" ? "Cancelled" : "Pending")} </Typography> </Box> */}
                  </Grid>
                  <Grid xs={12} sm={12} md={12} lg={12} container justifyContent='flex-end' textAlign='center' paddingTop='5%'  sx={{ display: 'flex', justifyContent: 'center' }}>
                    <Button onClick={() => {handleCancel()}} type="submit" sx={{ marginRight: '5%', paddingLeft: '10%', textIndent: '5.5px', paddingRight: '10%', borderRadius: '10px', backgroundColor: theme.palette.primary.main, '&:hover': { backgroundColor: "#2E0057" }, textTransform: 'none' }} variant="contained">Cancel Shift</Button>
