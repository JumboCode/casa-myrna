...
const CalendarModalButton: FC <any> = ({callback}) => {    
    ...
    // For Date/Time (Add Shift) validation
    const [timeError, setTimeError] = useState(true);

    // Allows for the update of the validation to happen in real-time despite asynchronosity of formdata
    useEffect(() => {
        validateTime();
    }, [formData]);
    ...
    const validateTime = () => {
        let startingDate = new Date(formData.startDate);
        let endingDate = new Date(formData.endDate);

        startingDate.setHours(formData.startTime)
        endingDate.setHours(formData.endTime)

        let timeDifference = endingDate.getTime() - startingDate.getTime();

        // Invalid time combination is if end time occurs before start time:
        if (timeDifference <= 0) {
            setTimeError(true);
        } else {
            setTimeError(false);
        ...
        <Box sx={style}>
            ...
            <Box>
                <form onSubmit={handleSubmit}>
                    <Collapse in={timeError}>
                        <Alert
                            severity="error"
                            sx={{ mb: 2 }}
                            >
                        Please enter a valid date/time combination.
                        </Alert>
                    </Collapse>
                                ...
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DatePicker 
                                        ...
                                        onChange={(newValue) => {
                                            ...
                                            handleInputChange({ target: { name: 'startDate', value: newValue } });
                                        }}
                                    ...
                                    <div component={['DatePicker']} >
                                        <DatePicker 
                                            ...
                                            onChange={(newValue) => {
                                                ...
                                                handleInputChange({ target: { name: 'endDate', value: newValue } });
                                            }} 
                                            sx={{ marginRight: '10px',width: '180px' }}
                                        />
                                    </div>
